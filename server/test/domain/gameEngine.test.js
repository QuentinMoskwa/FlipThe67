...
import {
    ...
    advanceToNextPlayer,
    finalizeRound,
    computeScoreForStayedPlayers,
} from '../../src/domain/gameEngine.js'
import { CardType, RoundPhase } from '../../src/domain/constants.js'
// ─── Mocks ────────────────────────────────────────────────────────────────────

vi.mock('../../src/domain/deck.js', () => ({ drawCard: vi.fn() }))
vi.mock('../../src/domain/logic/playerState.js', () => ({ isBust: vi.fn(), isFlipSeven: vi.fn() }))
vi.mock('../../src/domain/logic/scoring.js', () => ({ computeRoundScore: vi.fn(), updateCumulativeScores: vi.fn() }))
vi.mock('../../src/domain/logic/action.js', () => ({ resolveActionCard: vi.fn() }))
...
describe('computeScoreForStayedPlayers', () => {
    beforeEach(() => vi.clearAllMocks())

    it('calcule le score des joueurs stayed non bustés', () => {
        const round = makeRound(['p1', 'p2'])
        round.playerStates['p1'].hasStayed = true
        round.playerStates['p2'].hasStayed = false

        computeScoreForStayedPlayers(round)

        expect(computeRoundScore).toHaveBeenCalledTimes(1)
        expect(computeRoundScore).toHaveBeenCalledWith(round.playerStates['p1'])
    })

    it('ne calcule pas le score d\'un joueur busté même s\'il est stayed', () => {
    ...
    it('ne calcule pas le score des joueurs encore actifs', () => {
        const round = makeRound(['p1'])
...
describe('processSlay', () => {
    ...
    describe('carte Action', () => {
        ...
        it('appelle computeScoreForStayedPlayers après resolveActionCard', () => {
            const round = makeRound(['p1', 'p2'])
            drawCard.mockReturnValue(makeActionCard('freeze'))
            resolveActionCard.mockImplementation(() => {
    ...
    describe('carte NUMBER / MODIFIER', () => {
        ...
        it('retourne roundOver false et flipSeven false si rien de particulier', () => {
            ...
            expect(result).toEqual({ roundOver: false, flipSeven: false })
        })
        ...
        describe('Flip 7', () => {
            ...
            it('retourne roundOver true et flipSeven true', () => {
                ...
                expect(result).toEqual({ roundOver: true, flipSeven: true })
            })
...
describe('processStay', () => {
    ...
    it('retourne roundOver true si c\'était le dernier joueur actif', () => {
        ...
        const result = processStay(round, 'p1')
...
describe('advanceToNextPlayer', () => {
    it('passe au joueur suivant dans l\'ordre', () => {
        ...
        advanceToNextPlayer(round, players)
